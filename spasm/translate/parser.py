...
def set_var_argument(arguments: [], memory_pointer: int) -> int:
...
def split_argument_command(command_argument: str) -> list:
    ...
    return [commands[command_argument_list[0]]["code"], processor_result[0], processor_result[1]]
...
def write_output(file: str, data: list) -> None:
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python parser.py <source_filepath> <target_filepath>")
        sys.exit(1)

    source_filepath = sys.argv[1]
    target_filepath = sys.argv[2]

    result = run(source_filepath)
    write_output(target_filepath, result)
    reset_globals()

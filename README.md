...
- Первой инструкцией считается инструкция, с которой начинается метка "start"
- Все переменные должны быть объявлены в специальной секции "_data"
- Метки должны быть объявлены в отдельной строке
- Переход по метки означает переход на инструкцию последующую после ее объявления
- Исполняемый код и метки объявляются в секции "_programme"

## (ISA) Система команд

#### Набор исполняемых инструкций:

- ```push name_of_variable``` - положить значение переменной на вершину стека ```[...] => [..., value]```
- ```push value``` - положить значение на вершину стека ```[...] => [..., value]```
- ```pop``` - удалить верхний элемент стека ```[value_1, value_2] => [value_1]```
- ```incr``` - увеличить верхнее значения стека на 1 ```[..., value] => [..., value+1]```
- ```decr``` - уменьшить верхнее значения стека на 1 ```[..., value] => [..., value-1]```
...
- ```mod operand``` - находит остаток от деления верхнего значения стека и операнда и помещает на вершину стека ```[value] => [value, value%operand]```
...
Память инструкций и данных разделены согласно Гарвардской архитектуре

- В памяти данных длина машинного слова не ограничена
- В памяти инструкций она ограничена 8 байтами

Программист не имеет доступа к памяти инструкций

Поддерживаются различные типы данных:
- integer (от -2^32 до 2^32-1)
- char
- string

Порты ввода-вывода не отражены в памяти, для этого есть другое адресное пространство (для работы с этим присуствуют отдельные команды)
...
Поддерживается разная адрессации:
...
- Парсит метки, перменные и инструкции
- Базово проверяет синтаксис
- Транслирует код в набор простых команд и группирует их в двумерный массив ```[[opcode_1, operand_11, operand_12], [opcode_2, operand_21, operand_22], ...]```
...
### Control Unit
![CUnew.drawio.svg](scheme/CUnew.drawio.svg)

### Data path
![DatapathNew.drawio.svg](scheme/DatapathNew.drawio.svg)

### Примечание

- Процесс моделируется с отслеживанием по инструкции
...
- Доступ к памяти инструкций осуществляется по специальному регистру - command pointer
- Предусмотрены различные флаги АЛУ:
    1. ```Z``` (zero)- устанавливается когда результат последней арифметической операции равен 0
    2. ```N``` (negative) - устанавливается когда результат последней арифметической операции отрицателен 
...
- Файлы утилит находятся в [директории](spasm/golden/)
